...
async function createGatePass(req, res) {
  const body = req.body || {};
  if (!body.gpno || !body.gpdate || !body.dest) return res.status(400).json({ error: 'Fill GP No., Date and Destination first' });
...
async function deleteGatePass(id, res) {
  const { rowCount } = await query('DELETE FROM gate_passes WHERE id = $1', [id]);
