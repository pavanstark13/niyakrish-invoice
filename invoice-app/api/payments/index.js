...
async function listPayments(req, res) {
  ...
  const { rows: allocations } = await query(
    'SELECT * FROM payment_allocations WHERE payment_id = ANY($1)', [payments.map(p => p.id)]
  );
...
// Records a payment and applies it to invoices in one transaction: either a direct link to
// one invoice, or a general payment split across the customer's oldest unpaid invoices first.
// Every allocation is written to payment_allocations, so deletion later reverses exactly this
// — not a re-derived guess (see api/payments/[id].js).
async function createPayment(req, res) {
  const body = req.body || {};
  const { customerName, linkedInvoice, date, amount, paymentMode, refNo, notes } = body;
  if (!customerName || !date || !(amount > 0)) {
    return res.status(400).json({ error: 'Fill all mandatory details' });
  }
  ...
  const result = await withTransaction(async (client) => {
    ...
    if (linkedInvoice) {
      ...
      const { rows: clientInvoices } = await client.query(
        `SELECT * FROM invoices WHERE lower(customer_name) = lower($1) ORDER BY invoice_date ASC`,
        [customerName]
      );
